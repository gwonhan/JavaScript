...
function hello(name){
    ...
    //모든 인수는 암묵적으로 argument 객체의 프로퍼티로 보관이 된다.
...
//함수는 매개변수의 객수와 인수의 갯수가 일치하는지 체크하지 않는다.
//인수 (전달인자) 가 부족해서 할다되지 않은 매개변수의 값은 undefined 이다.
result = hello();
console.log(result);
...
function hi(name = '아무개'){
    ...
    if(arguments.length !==1 || typeof name !== 'string'|| name.length===0){
        throw new TypeError('인수는 1개이여야 하고, 문자열 값이며, 빈 문자열은 허용되지 않습니다. ');
    }
    return '${name} 안녕~~';
